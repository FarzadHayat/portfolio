...
const $$Index = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": "Blog - Bloomfolio", "description": "Read my latest blog posts and articles" }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<main class="max-w-6xl mx-auto px-4 py-20"> <!-- Header --> <div class="text-center mb-16"> <h1 class="text-5xl font-bold mb-4">Blog</h1> <p class="text-xl text-base-content/70">
...
</p> </div>` : renderTemplate`<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8"> ${sortedPosts.map((post) => renderTemplate`<article class="card bg-base-100 shadow-xl hover:shadow-2xl transition-shadow"> <figure class="aspect-video"> ${renderComponent($$result2, "Image", $$Image, { "src": post.data.image, "alt": post.data.title, "class": "w-full h-full object-cover", "width": 600, "height": 400 })} </figure> <div class="card-body"> <time class="text-sm text-base-content/60"> ${formatDate(post.data.publishDate)} </time> <h2 class="card-title hover:text-primary transition-colors"> <a${addAttribute(`/blog/${post.id}`, "href")}>${post.data.title}</a> </h2> <p class="text-base-content/80">${post.data.description}</p> ${post.data.tags && post.data.tags.length > 0 && renderTemplate`<div class="flex flex-wrap gap-2 mt-2"> ${post.data.tags.map((tag) => renderTemplate`${renderComponent($$result2, "SkillBadge", $$SkillBadge, { "skill": tag })}`)} </div>`} <div class="card-actions justify-end mt-4"> <a${addAttribute(`/blog/${post.id}`, "href")} class="btn btn-primary btn-sm">
Read More
</a> </div> </div> </article>`)} </div>`} </main> ` })}`;
}, "/home/farzad/portfolio/.worktrees/feat-quiz/src/pages/blog/index.astro", void 0);

const $$file = "/home/farzad/portfolio/.worktrees/feat-quiz/src/pages/blog/index.astro";
const $$url = "/blog";
